...
export default function CreateColumn({
  ...
  const chartDataShape = qDimensionInfo.length === 1 && qMeasureInfo.length === 1
  ...
  if (['both', 'yAxis'].some(substring => textOnAxis.includes(substring))) margin.left += 10
  ...
  if (chartDataShape === 'multipleDimensions') {
    ...
    if (chartWidth - padding * data.length > 0) {
      columnWidth = (chartWidth - padding * data.length) / data.length / qMeasureCount

      if (
        typeof columnPadding === 'undefined'
        && columnWidth < ColumnDefault.zoomScrollOnColumnWidth
      ) {
        columnWidth = (chartWidth - ColumnDefault.columnPaddingNarrow * data.length)
          / data.length
          / qMeasureCount
      ...
      columnWidth = (chartWidth - (ColumnDefault.columnPaddingNarrow / 2) * data.length)
          / data.length
          / qMeasureCount
  ...
  const isScrollDisplayed = (showScroll && columnWidth < ColumnDefault.zoomScrollOnColumnWidth)
  ...
  function createYAxis(diagram, data) {
    ...
    switch (chartDataShape) {
      ...
      case 'percentStacked':
        ...
        shiftForNegatives = showLabels === 'top'
    ...
    yValues.each(function (i) {
      ...
      const newValue = qMeasureInfo[0].qNumFormat.qType !== 'U'
        && qMeasureInfo[0].qNumFormat.qFmt.includes('%')
    ...
    if (
      ['solid', 'dashes', 'dots', 'default'].some(substring => showGridlines.includes(substring))
    ) {
      // Y Gridline
      const gridlines = d3
        .axisLeft()
  ...
  const getDimension = dim => qDataSet.filter(d => dim === d[Object.keys(d)[0]])[0]
  ...
  function handleMouseMove(d, i) {
    ...
    switch (chartDataShape) {
      ...
      case 'singleDimension':
        ...
        const toolTipValue = qMeasureInfo[measureNo].qNumFormat.qType === 'U'
      ...
      case 'multipleDimensions':
        ...
        const value = qMeasureInfo[0].qNumFormat.qType === 'U'
          || qMeasureInfo[0].qNumFormat.qFmt === '##############'
          || qMeasureInfo[0].qNumFormat.qFmt === '########'
          || qMeasureInfo[0].qNumFormat.qFmt === '###0'
          ? formatValue(d3.select(this).attr('data-value'))
  ...
  const yScaelCalc = (d, index) => (qDimensionCount === 1
  ...
  const drawColumnLabels = (columnLabels, measureName, index) => {
    const labelShift = showLabels === 'top'
  ...
  function setHeight(xAxisHeight = 0) {
    if (isScrollDisplayed) {
      ...
      heightOverview = +parseInt(heightValue, 10) - marginOverview.top - marginOverview.bottom
    } else {
    ...
    if (showLabels !== 'none') {
      ...
      const textValue = qMeasureInfo[0].qNumFormat.qType === 'U'
        || qMeasureInfo[0].qNumFormat.qFmt === '##############'
        || qMeasureInfo[0].qNumFormat.qFmt === '########'
        || qMeasureInfo[0].qNumFormat.qFmt === '###0'
        || chartDataShape === 'stackedChart'
      ...
      labelTextHeight = columnWidth >= sampleText.node().getBBox().width
        ? sampleText.node().getBBox().height
        : sampleText.node().getBBox().width

      labelTextHeight = showLabels === 'inside' ? labelTextHeight + 2 : labelTextHeight
      ...
      if (qMin < 0) {
        ...
        labelTextHeightNegative = columnWidth >= sampleText.node().getBBox().width
          ? sampleText.node().getBBox().height
          : sampleText.node().getBBox().width
  ...
  function shiftXAxis() {
    if (chartDataShape === 'multipleDimensions') {
      ...
      ticks.each(function (i) {
        ...
        const end = rects._groups[0][rects._groups[0].length - 1].x.baseVal.value
  ...
  function xAxisLabels() {
    ...
    xAxisHeight = xAxisLableSize.height - (isScrollDisplayed ? marginOverview.bottom : 0)
    ...
    if (['both', 'xAxis'].some(substring => textOnAxis.includes(substring))) {
      ...
      xAxisTextHeight = xAxisText.node().getBBox().height + (isScrollDisplayed ? 10 : 0)
    }
  ...
  function createLabels() {
    ...
    switch (chartDataShape) {
      case 'singleDimensionMeasure':
        ...
        break
      case 'singleDimension':
        const initialTransform = labelOrientation === 'horizontal'
          ? 0
          : columnWidth
              + labelTextWidth
  ...
  function brushed() {
    ...
    const columnRatio = (width - margin.right - legendLeftPadding) / (s[1] - s[0])

    if (chartDataShape === 'multipleDimensions') {
      const multiDimWidth = (width - margin.right - legendLeftPadding) * columnRatio
      columnWidth = multiDimWidth / (data.length + items.length)
      let noOfItems = 1
      let paddingItem = -1
      data.forEach((item, index) => {
