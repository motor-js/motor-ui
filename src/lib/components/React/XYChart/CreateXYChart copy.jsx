...
// const getDimension = (d) => d[0].qText; //AG
const getDimension = (d) => d.date;
...
export default function CreateXYChart({
  ...
  dataX.forEach((d) => {
    // console.log(d);
    const [a, b, c] = d;
    // const { date, city, tempearture } = d;
    // console.log(a, b, c);
    const date = a.qText;
    const city = b.qText;
    const temperature = c.qNum;
    console.log(date, city, temperature);
    // return;
    if (!temp.has(date)) {
      temp.set(date, { date, [`${city}`]: temperature });
    } else {
      temp.set(date, { ...temp.get(date), [`${city}`]: temperature });
    }
  });

  const data = Array.from(temp.values());
