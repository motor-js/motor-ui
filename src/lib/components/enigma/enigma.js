...
class Enigma {
  ...
  static configureDefaults(config) {
    if (!config) {
      // throw createEnigmaError(
      //   errorCodes.NO_CONFIG_SUPPLIED,
    ...
    if (!config.Promise && typeof Promise === "undefined") {
      // throw createEnigmaError(
      //   errorCodes.PROMISE_REQUIRED,
      //   "Your environment has no Promise implementation. You must provide a Promise implementation in the config."
      // );
    }

    if (
      typeof config.createSocket !== "function" &&
    ...
    config.protocol.delta =
      typeof config.protocol.delta !== "undefined"
        ? config.protocol.delta
        : true;
    // eslint-disable-next-line no-restricted-globals
