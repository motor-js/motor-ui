...
class RPC {
  ...
  open(force = false) {
    ...
    this.openedPromise = new this.Promise((resolve, reject) =>
      this.registerResolver("opened", null, resolve, reject)
    );
    this.closedPromise = new this.Promise((resolve, reject) =>
      this.registerResolver("closed", null, resolve, reject)
    );
    return this.openedPromise;
  }
  ...
  onClose(event) {
    ...
    this.rejectAllOutstandingResolvers(
      createEnigmaError(errorCodes.NOT_CONNECTED, "Socket closed")
    );
  }
  ...
  onError(event) {
    ...
    this.rejectAllOutstandingResolvers(
      createEnigmaError(errorCodes.NOT_CONNECTED, "Socket error")
    );
  }
  ...
  registerResolver(id, handle, resolve, reject) {
    ...
    resolver.on("resolved", (resolvedId) =>
      this.unregisterResolver(resolvedId)
    );
    resolver.on("rejected", (rejectedId) =>
      this.unregisterResolver(rejectedId)
    );
  }
