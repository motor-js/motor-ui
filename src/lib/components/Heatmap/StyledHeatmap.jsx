...
import {
  HeatmapWrapper,
  HeatmapWrapperNoData,
  HeatmapNoDataContent,
} from "./HeatmapTheme";
...
function StyledHeatmap(props) {
  ...
  // retrieve Heatmap data from HyperCube
  const {
    beginSelections,
    endSelections,
    qLayout,
  ...
  const handleResize = () => {
    if (typeof calcCond === "undefined" && dataError.length === 0) {
      // CreateHeatmap({ ...chartSettings, screenWidth: ref.current.offsetWidth });
    }
  };
    ...
    <>
      {data && qLayout && !dataError ? (
        ...
        <HeatmapWrapperNoData
          ...
          width={width}
        >
          <HeatmapNoDataContent height={height}>
