...
import combineBarBarStackData, {
  getStackValue,
} from "../../../utils/combineBarStackData";
...
>({
  ...
  const combinedData = useMemo(
    () =>
      combineBarBarStackData<XScale, YScale, Datum>(
        barSeriesChildren,
        horizontal
      ),
  ...
  const stackedData = useMemo(() => {
    const hasSomeNegativeValues =
      comprehensiveDomain.length > 0 &&
      comprehensiveDomain.some((num) => num < 0);
  ...
  const handleMouseMove = useCallback(
    (params: HandlerParams | undefined) => {
      ...
      dataKeys.forEach((key) => {
        ...
        const childData = barSeriesChildren.find(
          (child) => child.props.dataKey === key
        )?.props.data;
