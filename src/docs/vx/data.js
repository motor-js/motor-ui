...
import { theme } from "../../lib/components/VX/xy-chart/src";
...
// stacked data
export const groupKeys = Object.keys(cityTemperature[0]).filter(
  (attr) => attr !== "date"
);
...
priceBandData.points = priceBandData.band.map(({ x, y0, y1 }) => ({
  ...
  y:
    (y1 + y0) / 2 +
    (Math.random() > 0.5 ? -1 : 1) * Math.random() * ((y1 - y0) / 4),
}));
...
export const intervalLineData = cityTemperature.slice(0, 25).map((d, i) => ({
  ...d,
  x: d.date,
  y: intervals.some(([i0, i1]) => i >= i0 && i <= i1)
    ? null
    : Number(d[groupKeys[0]]),
}));
